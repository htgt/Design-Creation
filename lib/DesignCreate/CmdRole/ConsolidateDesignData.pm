...
{
    $DesignCreate::CmdRole::ConsolidateDesignData::VERSION = '0.047';
}
## use critic
...
sub modify_fusion_oligos {
    ...
    my $oligo_slice = {
        '1U5'   => sub { return $_[0] - (25 + $_[2]), $_[1] },
        '-1D3'  => sub { return $_[0] - (25 + $_[2]), $_[1] },
        '1D3'   => sub { return $_[0], $_[1] + (25 + $_[2]) },
        '-1U5'  => sub { return $_[0], $_[1] + (25 + $_[2]) },
    };
    ...
    foreach my $oligo (@oligos_arr) {
        ...
        foreach my $loci (@loci_array) {
            ...
            $self->log->debug($oligo->{type} . ' ' . "Start: " . $loci->{chr_start} . " End: " . $loci->{chr_end} . " Strand: " . $self->chr_strand . " Key: " . $self->chr_strand . $oligo->{type} . " " . $oligo->{seq});
            if ($oligo->{type} eq 'D3' || $oligo->{type} eq 'U5') {
                ...
                $self->log->debug($oligo->{type} . ' ' . "Start: " . $start_loc . " End: " . $end_loc);
                ...
                $self->log->debug($slice->seq);
                $loci->{chr_start} = $start_loc;
                $loci->{chr_end} = $end_loc;
                $oligo->{seq} = $slice->seq;
            }
            $self->log->debug($oligo->{type} . ' ' . $oligo->{seq});
        }
    }
